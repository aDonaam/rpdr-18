...
export async function getUserPageData(franchiseSlug, seasonNumber, usernameParam) {
  try {
    ...
    const { data: looks, error: looksError } = await supabaseAdmin
      .from("looks")
      .select("id, appearance_id, display_name, category_id")
    ...
    // Map looks by UUID; queen identity comes from the normalized appearance relation.
    const lookByUuid = {};
    looks.forEach((look) => {
    ...
    appearances.forEach((app) => {
      ...
      // Use image_path from season_appearances (authoritative), not from looks table
      queenInfoBySlug[slug] = {
        appearanceDisplayName: displayName,
        slug,
        image_path: app.image_path || null,
      };
    });
    ...
    for (const key in latestVoteByLookUser) {
      ...
      if (!look) continue;
      const appearance = appearances.find((app) => app.appearance_id === look.appearance_id);
      const s = queenStats[appearance?.queens?.slug];
      if (!s) continue;
