...
export async function getCategoryPageData(franchiseSlug, seasonNumber, categorySlug) {
  try {
    ...
    const appearanceNameBySlug = {};
    appearances.forEach((app) => {
      const slug = app.queens && app.queens.slug;
      if (!slug) return;
      appearanceNameBySlug[slug] = app.display_name || app.queens.display_name || "";
    });
    ...
    // Step 7: Attach approval stats + normalized appearance display name to each look
    const looks = looksRaw.map((look) => {
      const g = grouped[look.id];
      const appearanceDisplayName = appearanceNameBySlug[look.contestant_slug] || look.display_name || look.contestant_name || "";
      if (!g || g.total === 0) {
        return { ...look, overallApproval: null, overallVoteCount: 0, appearanceDisplayName };
      }
      const pct = Math.round((g.toot / g.total) * 100);
      return { ...look, overallApproval: pct, overallVoteCount: g.total, appearanceDisplayName };
    });
