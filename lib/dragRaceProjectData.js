...
import { getSeasonTheme } from "./seasonResolver";

/**
 * Fetch every season across every franchise, ordered by franchise then by
 * season number descending within each franchise.
 *
 * @returns {Promise<Array<{franchiseSlug, franchiseName, seasonNumber, theme}>>}
 */
export async function getAvailableSeasons() {
  const { data, error } = await supabaseAdmin
    .from("seasons")
    .select("season_number, theme_id, franchises(slug, name)")
