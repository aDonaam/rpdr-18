...
export async function getLooksPageData(franchiseSlug, seasonNumber) {
  try {
    ...
    const { data: looksData, error: looksError } = await supabaseAdmin
      .from("looks")
      .select("id, appearance_id, display_name, sequence, image_path, category_id")
    ...
    let looksWithStats = (looksData || []).map((look) => {
      ...
      const category = categoryById[look.category_id];
      const appearanceDisplayName = appearance?.display_name || appearance?.queens?.display_name || look.display_name || "";
      return {
        ...look,
        appearanceDisplayName,
        queenSlug: appearance?.queens?.slug || null,
    ...
    looksWithStats.sort((a, b) => {
      ...
      // Both have null sequence: alphabetize by normalized appearance name
